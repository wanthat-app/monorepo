...
`services/admin-api/src/guard.ts` (group re-check), and `infra/lib/admin-stack.ts` (authorizer).
...
- Admin HTTP API: `https://<admin-api-id>.execute-api.il-central-1.amazonaws.com` - called directly by the SPA (separate API from the customer app-api)
...
- Tokens live in **sessionStorage** (cleared on tab close); shorter refresh TTL (7 days) than
  customers. An admin session is structurally separate from a customer session - the admin-api
  authorizer trusts only the employee pool.
...
sequenceDiagram
    ...
    participant GW as Admin API Gateway JWT authorizer<br/>https://admin-api-id.execute-api<br/>.il-central-1.amazonaws.com
    participant AdminApi as admin-api Lambda (in-VPC)<br/>same execute-api domain
    participant Aurora as Aurora PG<br/>customers, audit log
    ...
    rect rgb(245, 245, 245)
        ...
        GW->>AdminApi: route with verified claims
        Note right of AdminApi: requireAdmin re-checks cognito groups contains "admin"<br/>(authorizer cannot gate on groups - defence in depth)
        alt stats / users / activity
            AdminApi->>Aurora: read customers, wallet stats, audit log<br/>(audit actor = email claim from the ID token)
        else config / catalog
            AdminApi->>DDB: read/write runtime_config, product, recommendation
        else customer administration
            AdminApi->>CustPool: AdminDeleteUser etc (scoped IAM grant)
            AdminApi->>Aurora: delete/update the customer row
        end
        AdminApi-->>SPA: response
    end
