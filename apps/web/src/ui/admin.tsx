    ...
    <aside className={`flex h-full w-[248px] shrink-0 flex-col border-e p-4 ${t.bg}`}>
      <div className="flex items-center gap-2.5 px-1.5 pt-1">
        ...
        <span className="flex flex-col">
          <span className={`font-display text-[21px] font-bold leading-none tracking-[-0.03em] ${t.text}`}>
            wanthat
          </span>
          <span className={`mt-0.5 text-[11px] font-semibold uppercase tracking-[0.14em] ${t.muted}`}>
            {caption}
          </span>
        </span>
      </div>
...
export function SidebarSection({ theme = "dark", children }: { theme?: SidebarTheme; children: ReactNode }) {
  return (
    <div className={`px-1.5 pb-2 pt-4 text-[10.5px] font-bold uppercase tracking-[0.13em] first:pt-0 ${SIDE_THEME[theme].muted}`}>
      {children}
    </div>
  );
}
  ...
  return (
    <div className={`flex items-center gap-2.5 rounded-field border p-2.5 px-3 ${t.card}`}>
      ...
      <span className="min-w-0 flex-1">
        ...
        {role ? <span className={`block text-[11px] ${t.muted}`}>{role}</span> : null}
      </span>
  ...
  return (
    <div className="rounded-[18px] border border-line bg-surface p-[18px]">
      <div className="flex items-center justify-between">
        ...
        <span className={`flex h-[30px] w-[30px] items-center justify-center rounded-[9px] ${tint}`}>{icon}</span>
      </div>
      ...
      {delta ? (
        <div className="mt-2.5 flex items-center gap-1.5">
          <span className={`rounded-full px-2 py-0.5 text-[11.5px] font-bold ${tint}`}>{delta}</span>
...
export function MerchantStatusChip({ tone, children }: { tone: MerchantStatusTone; children: ReactNode }) {
  const t = MERCHANT_TONES[tone];
  return (
    <span className={`inline-flex items-center gap-1.5 rounded-full px-2.5 py-1 text-xs font-bold ${t.chip}`}>
      ...
      <div className="flex flex-col gap-3.5">
        {items.map((s) => (
          <div key={s.label} className="flex items-center gap-2.5">
            <span aria-hidden className={`h-[11px] w-[11px] shrink-0 rounded-[3px] ${SWATCH[s.tone]}`} />
            ...
            {s.detail ? <span className="tabular w-14 text-end text-xs text-subtle">{s.detail}</span> : null}
          </div>
        ))}
      </div>
    </div>
...
export function SaveBar({
  ...
  return (
    <div className="sticky bottom-0 flex items-center gap-3 rounded-t-card border border-line bg-surface px-5 py-3.5">
      <span className={`flex items-center gap-2 text-[13px] font-semibold ${saved ? "text-accent" : "text-muted"}`}>
        {saved ? (
...
export function SearchField({
  ...
  return (
    <span
      ...
      style={{ width }}
    >
      <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#9aa8a1" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden>
...
export function NotificationBell({ hasAlert = false, onClick, label = "Notifications" }: { hasAlert?: boolean; onClick?: () => void; label?: string }) {
  return (
    <button
      type="button"
      onClick={onClick}
    ...
    >
      ...
      {hasAlert ? (
        <span aria-hidden className="absolute right-2.5 top-2 h-2 w-2 rounded-full border-2 border-base bg-accent" />
      ) : null}
    </button>
  );
}
