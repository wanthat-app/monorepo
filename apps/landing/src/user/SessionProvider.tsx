...
/**
 * React face of the session store (ADR-0006) — identical contract to the member app's. The
 * provider's only job is kicking off the one-time rehydrate (stored refresh token →
 * `InitiateAuth(REFRESH_TOKEN_AUTH)`, Cognito only — no backend); state lives in the
 * framework-free store so the module's actions can mutate it from anywhere.
 */
export interface Session {
  ...
  /** Current Cognito access token — the Bearer for the attributed resolve call. */
...
/**
 * A signed-in member's saved locale takes over the page language on rehydrate/sign-in (same
 * behaviour as the member app); signed out, the per-device choice the member app remembers
 * (read at i18n init) stays in charge.
 */
