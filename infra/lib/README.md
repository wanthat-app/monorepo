...
| `EdgeStack` (**us-east-1**) | TWO CloudFront distributions: the member site — **default → S3 SPA** (OAC, private, 403/404 → index.html) + **`/p/*` → landing HTTP API** (cross-region origin, caching disabled) — and the ADMIN console on its own origin (`admin.{domain}` → its own S3 bucket, strict-CSP response headers), sharing one CLOUDFRONT WAF web ACL; ACM cert (admin SAN) + Route 53 aliases; runtime **`/config.json`** into each SPA bucket; edge CloudWatch dashboard (`wanthat-{env}-edge`). app/admin APIs reached directly via Bearer, not fronted | 0019, 0016, 0007 |
