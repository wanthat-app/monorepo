...
| `EdgeServicesStack` | redirect Lambda (**non-VPC** → DynamoDB); conversion poller as a **non-VPC fetcher + in-VPC writer**; retailer fetcher(s) (non-VPC, secret-scoped); EventBridge Scheduler (configurable period) | 0007, 0009, 0008, 0004 |
...
- **No NAT Gateway and no RDS Proxy** (ADR-0003/0004). The only functions in the VPC are the ones
  ...
  endpoint and log out-of-band. Redirect and the retailer fetchers run outside the VPC, so nothing
