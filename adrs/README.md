...
landing); **0023** WhatsApp messaging; **0025** the canonical user identifier; **0026** interim
retailer API throttling; **0027** carries the full auth + customer-PII story (Cognito-native).
...
| [0003](0003-datastore-aurora-and-dynamodb.md) | Datastore: Aurora (money ledger only — PII lives in Cognito, 0006) + DynamoDB (redirect path) |
...
| [0019](0019-whatsapp-messaging-capability.md) | WhatsApp messaging capability: reusable `@wanthat/whatsapp` + Cognito Custom SMS Sender over AWS End User Messaging Social (WhatsApp-default OTP + `optin_welcome`); DynamoDB-Streams outbox NAT-free bridge (refines [0006](0006-cognito-native-auth-and-pii.md)) *(Proposed)* |
