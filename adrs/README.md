...
| [0024](0024-automatic-biometric-login-webauthn-conditional-ui.md) | Automatic biometric login: custom discoverable WebAuthn + conditional-UI autofill (passkey offers itself, one tap), our own assertion verification (`@wanthat/webauthn` + `passkey_credential` table), bridged to Cognito tokens via a server-side admin token exchange (ephemeral password + `ADMIN_USER_PASSWORD_AUTH` — ESSENTIALS pools reject CUSTOM_AUTH); button only when autofill unsupported (supersedes [0022](0022-faceid-passkey-authentication.md) login design) |
